...
管理画面に売上・平均注文単価・月別推移・商品別・決済手段別・クーポン別が出ます（`GET /api/admin/stats`）。

金額に入るのは **PAID / SHIPPED / DELIVERED** だけです。作られただけの `PENDING` を混ぜると、在庫を押さえただけの注文が売上に化けて、**辻褄は合っているのに実態より良い数字**が出続けます。一方でそれらを捨てもせず件数として出すので、**「作られた注文のうち何割が売上になったか」**が分かります（注文を消さずに状態で持つ設計の実利。予約をキャンセルで削除する clinic-reservation 側では原理的に出せない数字です）。

集計はすべて**注文時スナップショット**から積みます。カタログを join すると、値上げしただけで去年の売上が変わります。月の切り分けは店舗のタイムゾーン（`APP_STATS_ZONE`・既定 `Asia/Tokyo`）で行います（UTC のまま切ると毎月1日の午前9時までの売上が前月に入ります）。
...
- **QRコードはサーバーで描きます**（`QrCode.svg`・依存なしの純Java実装）。外部のQR生成サービスに投げないのは、**「この鍵をQRにして」と送るのは鍵を渡すのと同じ**だからです。読み取れない環境向けに、鍵の手入力と `otpauth://` リンクも併記しています

> **自作QRの正しさをどう担保したか。** 自作エンコーダには「実際にカメラで読めるか」を自動で確かめる手段がありません（デコーダも自作すれば同じ勘違いを2回するだけです）。そこで、**すでに実機で読めている `clinic-reservation/src/core/qr.php` からの移植**とし、**出力が1モジュールも違わないことをテストで突き合わせています**（`QrCodeTest`・4種の入力でモジュール行列のハッシュが一致）。⚠️ これは「移植で壊していない」ことしか保証しません——最終確認は実機のカメラで読むことでしかできません。
...
| 設定 | 効果 | 効かせている理由 |
|---|---|---|
| `APP_DEMO_READ_ONLY` | `/api/admin/**` への **POST/PUT/PATCH/DELETE を 403** | 入られても壊せない。**画像アップロード**（サーバーへのファイル書き込み＝最も強い権限）もここで止まる。判定は**HTTPメソッド**で行い、塞ぐ先の一覧を持たない——一覧方式は管理APIを足したときに書き漏らし、しかも漏れても何も起きないので気付けない |
| `APP_DEMO_MASK_CONTACT` | 管理画面の注文一覧で連絡先を `g***@example.com` に | **公開デモの実害リスクはここ**。誰でもゲスト購入で他人のアドレスを投入できる。⚠ **本人の照会では伏せません**（自分のアドレスが読めないのは保護ではなく不具合に見えるため） |
| `APP_DEMO_RETENTION_DAYS` | 一定日数を過ぎた注文から**連絡先だけ**消す | 溜め込まない。⚠ **注文自体は消しません**——消すと売上集計から過去が消え、「30日より前の売上が無かったことになる」という別の嘘を生みます |

デモに限らない対策として、**ログイン試行制限**（`app.auth.*`）も入れています。メールアドレス単位で数え、成功すれば即リセット。IP 単位にしないのは、送信元を替えるだけで抜けられ、`X-Forwarded-For` は詐称できるからです。代償として狙い撃ちの締め出しが可能になりますが、時間で自動的に解けるので上限つきの被害に収まります。

> 一般的な商用EC（Java 系に限らず）は、管理画面を**別ホスト**に置き、SSO+MFA、細かいロール分離、PII のマスクと監査ログ、アップロードは**別ドメインの S3/CDN** から配信、という構成を採ります。1VM 構成の本デモで現実的に効くのは「PII をマスクする」「配信オリジンを汚さない」「権限を絞る」の3つで、そこだけを取り込んだ形です。
...
| 管理: 統計 | `GET /api/admin/stats?months=` | ADMIN |
...
管理パネルの先頭には**売上の集計**が出ます。以下、**内税/外税トグル**・**税率テーブルの追加・改定・削除**・**送料の設定**・**クーポンの管理**・**商品画像の差し替え**ができます（「適用中」バッジはサーバーの実効税率判定と同じロジック＝終了日は排他的）。カートにはクーポン入力欄と金額内訳（小計 / 割引 / 送料 / 消費税 / 合計）が出ます。この数字は**サーバーの見積もりAPIがそのまま返したもの**です。
...
**起動時に自分で言います。** 既定値のままなら、起動完了後にまとめて警告が出ます（`docker run` 単体でも出ます）。

```
============================================================
 ⚠️  開発用の既定値のまま起動しています（2 件）
------------------------------------------------------------
 • APP_JWT_SECRET が既定値のままです。この状態では誰でも管理者になりすます…
 • APP_ADMIN_PASSWORD が既定値（README に記載の公開値）のままです。…
------------------------------------------------------------
 ローカルでの検証なら、このままで問題ありません。
============================================================
```

差し替えれば消えます。**消せない警告は出しません**——「シードが有効＝デモ用アカウントが作られる」は公開デモでは正しい設定で毎回真になるため、WARN ではなく `INFO [setup]` の1行に留めてあります。常時鳴る警告を混ぜると、本当に危ない2つまで読まれなくなります。
